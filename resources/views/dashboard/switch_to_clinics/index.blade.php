...
@endsection
@section('content')
<a href="{{ url('switch_to_clinics/create') }}">
    <button class="btn btn-success" style="margin-bottom: 25px;">
        <i class="fa fa-plus "></i> اضافة طلب تحويل لعيادة
    </button>
</a>

<div class="container mt-5">
    <h2 class="mb-4"> طلب التحويل</h2>
        ...
        function(event) {
            ...
            for (let i = 1; i < TRs.length; i++)
            {
                ...
                $(row).append(
                    ...
                    /**Edit**/
                    +
                    "<a style='display:block' class='dropdown-item'href='{{ url('switch_to_clinics/edit') }}" + "/" + datum.id +
                    "'> تعديل</a>" +
                    "<a style='display:block' class='dropdown-item'href='{{ url('switch_to_clinics/delete') }}" + "/" + datum.id +
                    "'>  حذف</a>" + "</div></div> </td>")
            }
        }
    });
