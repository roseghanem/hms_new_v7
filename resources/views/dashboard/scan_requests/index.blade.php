...
@endsection
@section('content')
<a href="{{ url('scan_requests/create') }}">
    <button class="btn btn-success" style="margin-bottom: 25px;">
        <i class="fa fa-plus "></i> اضافة طلب اشعة
    </button>
</a>

<div class="container mt-5">
    <h2 class="mb-4"> طلب الاشعة</h2>
    ...
    <table id="yajra-datatable" class="table table-bordered yajra-datatable">
        <thead>
            <tr>
                ...
                <th>الحمل</th>
                <th>تم تحضير المريض</th>
                <th>التاريخ</th>
                <th>كود الزيارة</th>
                <th>قسم الاشعة</th>
                <th>العضو المراد تصويره</th>
                <th></th>
...
<script type="text/javascript">
    ...
    function reload(value=null)
    {
        ...
        columns: [
            ...
            //edit
            {data: 'id', name: 'id'},
            {data: 'pregnant_woman', name: 'pregnant_woman'},
            {data: 'patient_preparation', name: 'patient_preparation'},
            {data: 'date', name: 'date'},
            {data: 'visit_id', name: 'visit_id'},
            {data: 'scan_unit_name', name: 'scan_unit_name'},
            {data: 'part_body_name', name: 'part_body_name'},
        ...
        function(event) {
            ...
            for (let i = 1; i < TRs.length; i++)
            {
                ...
                /* append name columns */
                $(row).append("<td> " + datum.pregnant_woman + " </td>")
                $(row).append("<td> " + datum.patient_preparation + " </td>")
                $(row).append("<td> " + datum.date + " </td>")
                $(row).append("<td> " + datum.visit_id + " </td>")

                $(row).append("<td> " + datum.scan_unit_name + " </td>")
                $(row).append("<td> " + datum.part_body_name + " </td>")
                $(row).append(
                    ...
                    /**Edit**/
                    +
                    "<a style='display:block' class='dropdown-item'href='{{ url('scan_requests/edit') }}" + "/" + datum.id +
                    "'> تعديل</a>" +
                    "<a style='display:block' class='dropdown-item'href='{{ url('scan_requests/delete') }}" + "/" + datum.id +
                    "'>  حذف</a>" + "</div></div> </td>")
            }
