...
@endsection
@section('content')
<a href="{{ url('drug_forms/create') }}">
    <button class="btn btn-success" style="margin-bottom: 25px;">
        <i class="fa fa-plus "></i> أضافة شكل صيدلاني    </button>
</a>

<div class="container mt-5">
    <h2 class="mb-4">الشكل الصيدلاني </h2>
    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        for (let i = 1; i < TRs.length; i++) {
            ...
            $(row).append(
                ...
                /**Edit**/
                +
                "<a style='display:block' class='dropdown-item'href='{{ url('drug_forms/edit') }}" + "/" + datum.id +
                "'> تعديل </a>" +
                "<a style='display:block' class='dropdown-item'href='{{ url('drug_forms/delete') }}" + "/" + datum.id +
                "'>  حذف </a>" + "</div></div> </td>")
        }
        },
