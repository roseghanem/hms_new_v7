...
@endsection
@section('content')
<a href="{{ url('analaysis_reqs/create') }}">
    <button class="btn btn-success" style="margin-bottom: 25px;">
        <i class="fa fa-plus "></i> اضافة طلب تحليل
    </button>
</a>

<div class="container mt-5">
    <h2 class="mb-4"> طلب تحليل مخبري</h2>
...
<script type="text/javascript">
    ...
    function reload(value=null)
    {
        ...
        columns: [
            ...
            {data: 'analaysis_category_name', name: 'analaysis_category_name'},
        ...
        function(event) {
            ...
            for (let i = 1; i < TRs.length; i++)
            {
                ...
                /* append name columns */
                $(row).append("<td> " + datum.notes + " </td>")
                $(row).append("<td> " + datum.date + " </td>")
                $(row).append("<td> " + datum.visit_id + " </td>")
                $(row).append("<td> " + datum.analaysis_category_name + " </td>")
                $(row).append(
                    ...
                    /**Edit**/
                    +
                    "<a style='display:block' class='dropdown-item'href='{{ url('analaysis_reqs/edit') }}" + "/" + datum.id +
                    "'> تعديل</a>" +
                    "<a style='display:block' class='dropdown-item'href='{{ url('analaysis_reqs/delete') }}" + "/" + datum.id +
                    "'>  حذف</a>" + "</div></div> </td>")
            }
